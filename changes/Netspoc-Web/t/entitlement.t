...
my $grid = $driver->find_element('//div[contains(@id, "accountview")]', 'xpath');
...
$driver->find_element_ok('//span[text()="Zuschauer (Watcher)"]',
												 'xpath', "found header:\t'Zuschauer (Watcher)'");
#grid is empty

$grid = $driver->find_element('//div[contains(@id, "supervisorlist")]', 'xpath');
...
$grid = $driver->find_element('//div[contains(@id, "supervisoremaillist")]', 'xpath');
ok($grid, "found grid:\tsupervisoremaillist");
$driver->find_element_ok('//span[contains(text(), "Berechtigte f")]',
												 'xpath', "found header:\tBerechtigte für x");
@check = ('guest');
...
sub check_grid {
	...
	my @list =
			$driver->find_child_elements($driver->find_element(
